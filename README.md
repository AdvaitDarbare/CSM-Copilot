...
User (conversational bar)
   ...
   Orchestrator Agent          ← routes intent, manages session
    ...
    └── Account Agent           ← questions about one specific account
         │
         ▼
   Account Intelligence Layer  ← shared account memory + decision layer
...
Tools: fetch merged context from the Account Intelligence Layer, generate brief, vector similarity query.
